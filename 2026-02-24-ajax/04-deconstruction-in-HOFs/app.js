...
// same with deconstruction
console.log(pizzas.find(({ price }) => price < 30))

// all pizzas with diameter under 20
console.log(pizzas.filter(pizzaFromOven => pizzaFromOven.diameter < 20))
// with deconstruction:
console.log(pizzas.filter(({ diameter }) => diameter < 20))
...
// deconstruction exmaple:
console.log(pizzas.map(({ price }) => {
    return {
        originalPrice: price,
        includingVAT: price * 1.18
    }
}))
...
// with deconstruction:
console.log(pizzas.reduce((max, { price }) => price > max ? price : max, 0))
